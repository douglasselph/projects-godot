...
var _quadArea: MeshInstance
var _touchArea: Area

func _init(source: Spatial, viewport: Viewport, quadArea: MeshInstance, touchArea: Area):
	_source = source
	_viewport = viewport
	_quadArea = quadArea
	_touchArea = touchArea
	_touchArea.connect("mouse_entered", self, "_mouse_entered_touch_area")
	_touchArea.connect("mouse_exited", self, "_mouse_exited_touch_area")
...
func _findMousePosInArea(global_position: Vector2) -> Vector3:
	var camera = _source.get_viewport().get_camera()
	...
	var result = _source.get_world().direct_space_state.intersect_ray(from, to, [], _touchArea.collision_layer,false,true) #for 3.1 changes
