...
func applyParams():
	...
	translation = Vector3(TerrainPos.x + halfSize.x, 0, TerrainPos.y + halfSize.y)
	...
	material.set_shader_param("meshGlobalSize", TerrainGlobalSize)
	...
	material.set_shader_param("meshOffset", offset)
	
	print(TerrainPos, ": offset=", offset, ", translation=", String(translation), ", size=", String(mesh.size))
	
	material.set_shader_param("amplitude", Amplitude)
	material.set_shader_param("exponentiation", Exponentiation)
